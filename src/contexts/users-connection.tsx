...
import { VideoPeer } from "@/components/index";
...
export function UsersConnectionProvider({ stream, peer, myId, children }: any) {
    ...
    socket.on(
      ...
      ({ id, name, picture, muted: initMuted, visible: initVisible }: any) => {
        ...
        call.on("stream", (stream: MediaStream) => {
          setStreams(
            append({
              [id]: <VideoPeer stream={stream} isMe={false} name={name} />,
            })
          );
  ...
  useEffect(() => {
    ...
    peer.on("call", (call: any) => {
      ...
      call.on("stream", (stream: MediaStream) => {
        setStreams(
          append({
            [peer]: <VideoPeer stream={stream} isMe={false} name={user.name} />,
          })
        );
        const screenTrack = stream.getVideoTracks()[1];
        if (screenTrack) setSharedScreenTrack(screenTrack);
